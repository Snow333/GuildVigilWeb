...
export function combatant(over: Partial<Combatant> = {}): Combatant {
  return {
    ...
    // Brief #20: Medium by default, so every existing assertion in this suite
    // measures exactly what it measured before radius existed. Pass
    // `radius: 0.5` to build a Large body.
    radius: 0,
